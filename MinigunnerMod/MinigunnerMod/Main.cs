using BepInEx;
using MinigunnerMod.Artifact;
using MinigunnerMod.Equipment;
using MinigunnerMod.Equipment.EliteEquipment;
using MinigunnerMod.Items;
using R2API;
using R2API.Utils;
using System;
...
namespace MinigunnerMod
{
    [BepInPlugin(ModGuid, ModName, ModVer)]
    [BepInDependency(R2API.R2API.PluginGUID, R2API.R2API.PluginVersion)]
    [NetworkCompatibility(CompatibilityLevel.EveryoneMustHaveMod, VersionStrictness.EveryoneNeedSameModVersion)]
    [R2APISubmoduleDependency(nameof(ItemAPI), nameof(LanguageAPI), nameof(EliteAPI), nameof(PrefabAPI), nameof(ContentAddition))]
    public class Main : BaseUnityPlugin
    {
        public const string ModGuid = "com.MinigunnerDev.MinigunnerMod";
        public const string ModName = "Minigunner Mod";
        ...
        private void Awake()
        {
            ...
            using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("MinigunnerMod.minigunnermodassetbundle"))
            {
